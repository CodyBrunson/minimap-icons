...
The built plugin will be available in the `dist/` directory as `MinimapIcons.js`.
...
Example-Plugin/
├── src/
│   ├── MinimapIcons.ts    # Main plugin class
...
1. **Rename your plugin**: Update the `pluginName` and `author` properties in `src/MinimapIcons.ts`
...
3. **Copy the built plugin**:
   ...
   ```bash
   cp dist/MinimapIcons.js /path/to/HighLiteDesktop/src/renderer/client/plugins/
   ```
